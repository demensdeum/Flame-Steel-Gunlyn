...
export class InitializationScreenState {
    ...
    initialize() {
        ...
        this.context.sceneController.objectPlayAnimation("kokeshi", "Плоскость.004|Scene");
    }
    showGeolocationPreloader() {
    ...
    step() {
        if (window.localStorage.getItem("gameRulesAccepted") != "YES" &&
        ...
        else if (window.localStorage.getItem("gameRulesAccepted") == "YES" &&
    ...
    serverInfoControllerDidFetchInfo(_, entries) {
    ...
    authorizeControllerDidAuthorize(_, heroUUID) {
        if (heroUUID) {
    ...
    authorizeControllerDidReceiveError(_, message) {
        _alert({
